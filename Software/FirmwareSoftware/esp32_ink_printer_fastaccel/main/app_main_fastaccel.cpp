...
#define FAS_ENGINE_CORE             0
#define FAS_DIR_CHANGE_DELAY_US     200U
#define FAS_ENABLE_DELAY_US         200U
#define FAS_DISABLE_DELAY_MS        10U
#define FAS_DEFAULT_ACCEL_HZ_S      20000U
#define FAS_HOME_ACCEL_HZ_S         8000U
#define FAS_WAIT_SLICE_MS           2U
#define FAS_SEARCH_TRAVEL_MM        400.0f
#define FAS_HOME_SLOW_FACTOR        3.0f
#define FAS_MIN_MOVE_TIME_S         0.001f
#define FAS_ACCEL_SPEED_MULT        4U
...
static FastAccelStepperEngine g_stepper_engine = FastAccelStepperEngine();
static FastAccelStepper *g_stepper[AXIS_COUNT] = {nullptr};

static void motion_abort_all(void);
static esp_err_t stepper_motion_init(void);
...
static bool motion_stop_requested(void) {
    bool stop_requested;
    xSemaphoreTake(g_state_mutex, portMAX_DELAY);
...
static void sync_state_positions_from_steppers(void) {
    xSemaphoreTake(g_state_mutex, portMAX_DELAY);
    for (int i = 0; i < AXIS_COUNT; ++i) {
        if (g_stepper[i]) {
            g_state.pos_steps[i] = g_stepper[i]->getCurrentPosition();
        }
    }
    xSemaphoreGive(g_state_mutex);
}

static uint32_t mm_min_to_speed_hz(axis_id_t axis, float feed_mm_min) {
    float hz = fabsf(feed_mm_min) * g_axes[axis].steps_per_mm / 60.0f;
    if (hz < 1.0f) hz = 1.0f;
    return (uint32_t)lroundf(hz);
}

static uint32_t accel_for_speed(uint32_t speed_hz) {
    uint32_t accel = speed_hz * FAS_ACCEL_SPEED_MULT;
    if (accel < FAS_DEFAULT_ACCEL_HZ_S) accel = FAS_DEFAULT_ACCEL_HZ_S;
    return accel;
}
...
static void motion_abort_all(void) {
    for (int i = 0; i < AXIS_COUNT; ++i) {
        if (g_stepper[i]) {
            int32_t pos = g_stepper[i]->getCurrentPosition();
            g_stepper[i]->forceStopAndNewPosition(pos);
            g_stepper[i]->disableOutputs();
        }
        set_axis_enable((axis_id_t)i, false);
    }
    sync_state_positions_from_steppers();
}

static esp_err_t stepper_motion_init(void) {
    g_stepper_engine.init(FAS_ENGINE_CORE);
    ...
    for (int i = 0; i < AXIS_COUNT; ++i) {
        ...
        g_stepper[i] = g_stepper_engine.stepperConnectToPin((uint8_t)g_axes[i].step_pin);
        if (!g_stepper[i]) {
            ESP_LOGE(TAG, "FastAccelStepper connect failed for axis %d on step pin %d", i, g_axes[i].step_pin);
            return ESP_FAIL;
        }

        if (pin_is_valid(g_axes[i].dir_pin)) {
            g_stepper[i]->setDirectionPin((uint8_t)g_axes[i].dir_pin,
                                          !g_axes[i].invert_dir,
                                          FAS_DIR_CHANGE_DELAY_US);
        }

        if (pin_is_valid(g_axes[i].en_pin)) {
            g_stepper[i]->setEnablePin((uint8_t)g_axes[i].en_pin, g_axes[i].enabled_low);
            g_stepper[i]->setAutoEnable(true);
            g_stepper[i]->setDelayToEnable(FAS_ENABLE_DELAY_US);
            g_stepper[i]->setDelayToDisable(FAS_DISABLE_DELAY_MS);
        }

        g_stepper[i]->setAcceleration(FAS_DEFAULT_ACCEL_HZ_S);
        g_stepper[i]->setSpeedInHz(mm_min_to_speed_hz((axis_id_t)i, DEFAULT_FEED_MM_MIN));
        g_stepper[i]->setCurrentPosition(0);
        g_stepper[i]->disableOutputs();
    }
    return ESP_OK;
}

static esp_err_t wait_for_active_axes(const bool axis_active[AXIS_COUNT]) {
    while (1) {
        bool any_running = false;
        for (int i = 0; i < AXIS_COUNT; ++i) {
            if (axis_active[i] && g_stepper[i] && g_stepper[i]->isRunning()) {
                any_running = true;
                break;
            }
        }
        if (!any_running) return ESP_OK;
        if (machine_faulted() || motion_stop_requested()) return ESP_FAIL;
        vTaskDelay(pdMS_TO_TICKS(FAS_WAIT_SLICE_MS));
    }
}

static esp_err_t wait_for_axis_or_switch(axis_id_t axis, gpio_num_t watched_switch) {
    while (g_stepper[axis] && g_stepper[axis]->isRunning()) {
        if (machine_faulted() || motion_stop_requested()) return ESP_FAIL;
        if (pin_is_valid(watched_switch) && is_switch_triggered(watched_switch)) {
            int32_t pos = g_stepper[axis]->getCurrentPosition();
            g_stepper[axis]->forceStopAndNewPosition(pos);
            while (g_stepper[axis]->isRunning()) {
                vTaskDelay(pdMS_TO_TICKS(FAS_WAIT_SLICE_MS));
            }
            return ESP_OK;
        }
...
static esp_err_t move_linear_steps(const int32_t target_steps[AXIS_COUNT], float feed_mm_min) {
    ...
    for (int i = 0; i < AXIS_COUNT; ++i) {
        start[i] = g_stepper[i] ? g_stepper[i]->getCurrentPosition() : g_state.pos_steps[i];
    }
    ...
    for (int i = 0; i < AXIS_COUNT; ++i) {
        ...
        if (axis_active[i] && !g_stepper[i]) {
            ESP_LOGE(TAG, "Axis %d requested to move but has no FastAccelStepper backend", i);
    ...
    if (path_mm < FAS_MIN_MOVE_TIME_S) {
        path_mm = fmaxf(fabsf(dx) + fabsf(dy) + fabsf(dz) + fabsf(de), FAS_MIN_MOVE_TIME_S);
    }

    float move_time_s = path_mm / (feed_mm_min / 60.0f);
    if (move_time_s < FAS_MIN_MOVE_TIME_S) move_time_s = FAS_MIN_MOVE_TIME_S;

    for (int i = 0; i < AXIS_COUNT; ++i) {
        if (!axis_active[i]) continue;
        uint32_t axis_speed_hz = (uint32_t)lroundf((float)abs_delta[i] / move_time_s);
        if (axis_speed_hz < 1U) axis_speed_hz = 1U;

        g_stepper[i]->setSpeedInHz(axis_speed_hz);
        g_stepper[i]->setAcceleration(accel_for_speed(axis_speed_hz));
        auto move_result = g_stepper[i]->moveTo(target_steps[i], false);
        if ((int)move_result != 0) {
            ESP_LOGE(TAG, "FastAccelStepper moveTo failed on axis %d with code %d", i, (int)move_result);
            ret = ESP_FAIL;
            goto cleanup;
        }
    }
    ...
    while (1) {
        ret = wait_for_active_axes(axis_active);
...
static esp_err_t home_single_axis(axis_id_t axis) {
    ...
    const axis_hw_t *a = &g_axes[axis];
    FastAccelStepper *stepper = g_stepper[axis];
    if (a->min_pin == GPIO_NUM_NC) return ESP_ERR_NOT_SUPPORTED;
    if (!stepper) return ESP_ERR_NOT_SUPPORTED;

    const int32_t backoff_steps = mm_to_steps(axis, HOMING_BACKOFF_MM);
    const int32_t search_steps = mm_to_steps(axis, FAS_SEARCH_TRAVEL_MM);
    const int32_t slow_seek_steps = mm_to_steps(axis, HOMING_BACKOFF_MM * 2.0f);
    const int search_sign = a->home_to_min ? -1 : 1;
    const int away_sign = -search_sign;
    const uint32_t fast_hz = mm_min_to_speed_hz(axis, HOMING_FEED_MM_MIN);
    const uint32_t slow_hz = mm_min_to_speed_hz(axis, HOMING_FEED_MM_MIN / FAS_HOME_SLOW_FACTOR);

    stepper->setAcceleration(FAS_HOME_ACCEL_HZ_S);
    stepper->setSpeedInHz(fast_hz);

    if (is_switch_triggered(a->min_pin)) {
        auto mr = stepper->move(away_sign * backoff_steps, false);
        if ((int)mr != 0) return ESP_FAIL;
        if (wait_for_axis_or_switch(axis, GPIO_NUM_NC) != ESP_OK) {
            motion_abort_all();
            return ESP_FAIL;
        }
    }

    auto mr = stepper->move(search_sign * search_steps, false);
    if ((int)mr != 0) return ESP_FAIL;
    if (wait_for_axis_or_switch(axis, a->min_pin) != ESP_OK) {
        motion_abort_all();
        return ESP_FAIL;
    }
    if (!is_switch_triggered(a->min_pin)) {
    ...
    mr = stepper->move(away_sign * backoff_steps, false);
    if ((int)mr != 0) return ESP_FAIL;
    if (wait_for_axis_or_switch(axis, GPIO_NUM_NC) != ESP_OK) {
        motion_abort_all();
        return ESP_FAIL;
    ...
    stepper->setSpeedInHz(slow_hz);
    mr = stepper->move(search_sign * slow_seek_steps, false);
    if ((int)mr != 0) return ESP_FAIL;
    if (wait_for_axis_or_switch(axis, a->min_pin) != ESP_OK) {
        motion_abort_all();
        return ESP_FAIL;
...
static void ui_textarea_event_cb(lv_event_t *e) {
    if (lv_event_get_code(e) == LV_EVENT_FOCUSED || lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ui_set_active_ta((lv_obj_t *)lv_event_get_target(e));
    }
}
...
static esp_err_t ui_display_touch_init(void) {
    ...
    buscfg.max_transfer_sz = TFT_H_RES * 40 * sizeof(uint16_t);
    ...
    io_config.pclk_hz = TFT_PIXEL_CLOCK_HZ;
    ...
    if (PIN_TFT_BCKL != GPIO_NUM_NC) {
        gpio_config_t bk = {};
        bk.pin_bit_mask = 1ULL << PIN_TFT_BCKL;
        bk.mode = GPIO_MODE_OUTPUT;
        bk.pull_up_en = GPIO_PULLUP_DISABLE;
        bk.pull_down_en = GPIO_PULLDOWN_DISABLE;
        bk.intr_type = GPIO_INTR_DISABLE;
    ...
    if (PIN_TOUCH_CS != GPIO_NUM_NC) {
        ...
        tp_cfg.x_max = TFT_H_RES;
        tp_cfg.y_max = TFT_V_RES;
    ...
    disp_cfg.io_handle = g_lcd_io;
    disp_cfg.panel_handle = g_lcd_panel;
    disp_cfg.buffer_size = TFT_DRAW_BUF_PIXELS;
    disp_cfg.double_buffer = true;
    disp_cfg.hres = TFT_H_RES;
    disp_cfg.vres = TFT_V_RES;
    ...
    if (g_touch) {
        ...
        touch_cfg.disp = g_lv_disp;
        touch_cfg.handle = g_touch;
...
extern "C" void app_main(void) {
    ...
    gpio_init_all();
    ESP_ERROR_CHECK(stepper_motion_init());
